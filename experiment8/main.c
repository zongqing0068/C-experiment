...
int main()
{
    ...
    int n, t, i, j, p, ichoice, isum, iaver;//n为学生人数，t为考试科目数，iscore为学生成绩，isum为总成绩，iaver为平均成绩
    ...
    for (i = 0;; i++)
    {
        ...
        while (scanf("%d", &ichoice) != 1 || ichoice < 0 || ichoice > 11 || getchar() != '\n')//防止类型错误，或者在正确输入值后紧跟其他字符，或者输入多个字符
        {
            while (getchar() != '\n');
            printf("error\n");
